...
| `pnpm verify:docs` | Load a sample of built routes in real Chrome, light + dark (the shell's own toggle, checked on `<html>`), and fail on a blank page, a console error, a failed chunk, a stuck Suspense fallback, or a theme that did not apply. `--all` for every route; `--shots <dir>` also writes a full-page screenshot per route and theme. Needs a prior `pnpm build`. |
| `pnpm shots:compare <baseline> <current>` | Diff two screenshot folders (pixelmatch) and list the routes that changed; `--out <dir>` writes diff images, `--summary <file>` appends a Markdown table. CI runs it on every pull request against the last green run on `main`. |
...
`gen-contracts` fails on an unknown key or an id with no section, so a typo can't silently drop the guidance someone wrote. Thirteen components are authored so far — button, input, select, badge, card, dialog, sheet, table, sidebar, toast, alert-dialog, hover-card and application-shell; the rest carry the derived half only. A section in **any** group is its primitive's page — the application shell's is under Blocks, the logo's under Foundations — and each contract also lists `aliases`: the ids a design agent plausibly writes that resolve to it (its file name where that differs, such as `sonner` for toast, and the kebab-case of its first exported part where none is named for it, such as `chart-container` for chart). The validator accepts an alias silently; the id stays the one to cite. `contracts.json`'s own `counts` block is the number of record, not this sentence. Everything written there is published to consumers — through the design specs, and directly — so each claim has to be true of the primitive or stated in `design.md`.
...
- **`pnpm verify:docs`** — the closest thing to a test suite here. `pnpm build` only proves the export prerenders; the page shells are empty and every section renders client-side, so a build can succeed while the site renders nothing. This loads ten representative routes in system Chrome, in **both themes**, and fails on a blank page, a console error, a failed chunk, or a Suspense fallback that never resolved. Run it after anything that changes how the docs site loads. The docs shell keeps its own theme state and ignores `prefers-color-scheme`, so the dark pass clicks the shell's toggle and checks `<html>` carries `.dark`; before that check existed, the "dark" pass ran light and nobody knew.
- **Screenshot baselines** — `verify:docs` proves a page renders, not that it looks right; two card defects passed every gate and were caught only by someone rendering them by hand. CI now runs `pnpm verify:docs --all --shots shots` on every run, keeps the screenshots as the `docs-shots` artifact for 30 days, and on a pull request diffs them against the last green run on `main` with `pnpm shots:compare`: the changed routes land in the job summary with the fraction of pixels that moved, and the diff images in a `docs-shot-diffs` artifact. It reports and never fails — a visual change is usually the point of the PR, and the reviewer is the gate. Locally: `pnpm verify:docs --all --shots shots` before and after a change (into two folders), then `pnpm shots:compare before after --out diffs`.
