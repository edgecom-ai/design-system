...
const PopoverSearchDemo = () => {
  ...
  // Loading while the debounced value is catching up to the latest input.
  const isLoading = inputValue !== debouncedSearch

  // Derive the filtered list from the debounced search term during render.
  const searchTerm = debouncedSearch.trim().toLowerCase()
  const filteredUsers =
    searchTerm === '' ? users : users.filter(user => user.name.toLowerCase().includes(searchTerm))
        ...
        <div className='grid gap-4'>
          ...
          <ul className='space-y-2'>
            {filteredUsers.length > 0 ? (
              ...
              <li className='py-2 text-center text-sm'>No team members found</li>
            )}
          </ul>
        </div>
      </PopoverContent>
