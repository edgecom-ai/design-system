...
import { ZapIcon, GaugeIcon, MonitorSmartphoneIcon, SearchIcon, MoreVerticalIcon, Undo2Icon, ArrowUpIcon, ArrowDownIcon } from "lucide-react"
...
const SearchDialog = ({ defaultOpen = false, trigger, className }: Props) => {
  ...
  const suggestions = [
    ...
    },
    {
      ...
      icon: (
        <ZapIcon className='text-foreground size-4.5!' />
      ),
      label: 'Consumption overview (dataTrack™)'
    },
    {
      id: 'neuracharge-stations',
      icon: (
        <MonitorSmartphoneIcon className='text-foreground size-4.5!' />
      ),
      label: 'NeuraCharge™ stations'
    }
  ...
  const interactions = [
    ...
    },
    {
      id: 'dr-event-418',
      name: 'DR Event #418',
      description: 'Demand Response · curtailment dispatch active',
  ...
  const users = [
    {
      id: 'barry-barnett',
      name: 'Barry Barnett',
      email: 'Site manager · Downtown Substation',
  ...
  return (
    <div className={className}>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        >
          <DialogTitle className='sr-only'>Search Edgecom</DialogTitle>
          <Combobox open={true}>
            <ComboboxInput
            ...
            <ComboboxList className='border-0'>
              <ComboboxEmpty>No sites, meters, or reports match your search.</ComboboxEmpty>

              {suggestions.length > 0 && (
                <>
                  <ComboboxGroup className='p-4!'>
                    <ComboboxLabel>Quick links</ComboboxLabel>
              ...
              {interactions.length > 0 && (
                <>
                  <ComboboxGroup className='p-4!'>
                    <ComboboxLabel>Sites & events</ComboboxLabel>
              ...
              {users.length > 0 && (
                <ComboboxGroup className='p-4!'>
                  <ComboboxLabel>People</ComboboxLabel>
                  {users.map(user => (
                    <ComboboxItem
                      key={user.id}
                      value={user.id}
