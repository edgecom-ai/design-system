...
const CommandEmpty = ({ className, ...props }: React.ComponentProps<typeof CommandPrimitive.Empty>) => {
  ...
  return <div className={cn('px-2 py-4 text-center text-body-sm', className)} cmdk-empty='' role='presentation' {...props} />
}
...
const MultipleSelector = ({
  ...
  // Sync the controlled `value` prop into local state during render (avoids a
  // set-state-in-effect cascade) using React's "adjust state on prop change" pattern.
  ...
  // Derive grouped options from `arrayOptions` during render (unless `onSearch`
  // owns the option list). Tracking the previous serialized input keeps this a
  // render-time adjustment rather than a set-state-in-effect cascade.
  const nextGroupedOptions = React.useMemo(
    () => (!arrayOptions || onSearch ? null : transToGroupOption(arrayOptions, groupBy)),
    [arrayOptions, groupBy, onSearch]
  )

  if (nextGroupedOptions && JSON.stringify(nextGroupedOptions) !== JSON.stringify(options)) {
    setOptions(nextGroupedOptions)
  }
  ...
  return (
    ...
    >
      <div
        className={cn(
          'border-input focus-within:border-ring focus-within:ring-ring/50 has-aria-invalid:ring-destructive/20 dark:has-aria-invalid:ring-destructive/40 has-aria-invalid:border-destructive relative min-h-9.5 rounded-md border text-body-sm transition-[color,box-shadow] outline-none focus-within:ring-[3px] has-disabled:pointer-events-none has-disabled:cursor-not-allowed has-disabled:opacity-50',
          {
