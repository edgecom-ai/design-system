...
function Slot({ children, ...props }: HTMLAttributes<HTMLElement>) {
  return useRender({
    render: (isValidElement(children) ? children : <span />) as ReactElement<
...
export interface SortableRootProps<T> extends Omit<HTMLAttributes<HTMLDivElement>, 'onDragStart' | 'onDragEnd'> {
  value: T[]
...
function Sortable<T>({
  ...
  // Client-only mount guard so the `createPortal(..., document.body)` overlay below
  // never runs during SSR. This is the intended use of setState-in-effect.
  ...
  const overlayContent = useMemo(() => {
    ...
    Children.forEach(children, child => {
      if (
        isValidElement<{ value?: UniqueIdentifier; className?: string }>(child) &&
...
function SortableItem({ value, className, asChild = false, disabled, children, ...props }: SortableItemProps) {
