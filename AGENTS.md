...
- **Destructive actions require a double confirmation — never one-click.** Any destructive or irreversible action (delete, remove, disconnect, reset, purge) must pass through an explicit confirmation before it runs — use the `alert-dialog` with a `destructive`-variant confirm button and copy that names what's affected and that it can't be undone (the trigger + the deliberate confirm are the two steps). Never wire a destructive action straight to its trigger. For high-impact / bulk / irreversible actions, add a stronger second barrier (type-to-confirm the resource name). Surface the outcome with a `toast` afterward.
