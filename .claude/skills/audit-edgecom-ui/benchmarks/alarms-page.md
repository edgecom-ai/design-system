...
| 2026-09-25 | v1.1.0 · `0219b54b` | 12 / 13 | 12 | Fresh project, README only, the first run on a released version. Every run-2 fix took: the manifest cites the live identity, data-driven badges list their options as arrays, looped skeletons and checkboxes stamp one literal id, no Recharts element sets `fontSize`, the reading chart is an event-window chart with only the export menu, and the phone filters moved into a bottom sheet. Strict errors fell from ten to five, and they are two defects: `side` recorded as a variant on three sheets (a prop, which the README now names), and a chart the logic builds carrying its id on a wrapper `<div>` (the README now shows the props form; the validator reports it once, not twice). The render found one defect of ours: a left or right sheet was three-quarters wide on a phone instead of the full width `design.md` specifies — fixed, which also gives the mobile sidebar back its 18rem. The KPI row is still three columns at 390, written as an inline `grid-template-columns` this time; the README now lists the grid steps and says an inline grid cannot respond. |
