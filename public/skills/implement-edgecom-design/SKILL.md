...
1. **Read the design** through the Claude Design MCP. If the design project carries a `_system.json`, read it too — it names the design-system `version`, commit and `digest` the project was generated from.
2. **Check versions.** Compare that pair with `contracts/index.json`'s `version` and `digest`; report a mismatch before building. Then compare the consuming app's `globals.css` token values with `tokens.json` — the `theme` is a copy-on-install snapshot, not a live link, so a design generated against current tokens can meet an app holding an older set. If stale, re-sync with `pnpm dlx shadcn@latest add edgecom-ai/design-system/theme --overwrite` first.
3. **Resolve each designed element to a registry component** from `contracts/index.json` before writing any layout — match on `summary`, `purpose`, `useWhen` and `dontUseWhen`, and pick the variant and size from `variants`. A variant not in that list does not exist. Don't build freeform and retrofit components onto it.
...
8. **Cover the states the design implies** — loading, empty, error, success — plus light/dark and mobile/desktop. The contract's `behavior` says what each primitive does in those paths.
...
- `edgecom ds search|inspect|validate-design|audit` structured commands. Today the equivalent is fetching `contracts/index.json` and the per-component contracts directly.
- The design handoff manifest — a file beside a design naming the system version it was built against and the component identity of each element. Designs produced today carry no such manifest, so verify version through the project's `_system.json` and component identity by reading the design directly.
